...
// ---------------------------------------------------------------------------
// ASLR support: the game's PE has DYNAMIC_BASE set, so it may load at any
// base address.  All hard-coded addresses in this plugin assume the default
// image base of 0x00400000.  aslr() adds the delta at runtime so every
// address resolves correctly regardless of where the OS placed the EXE.
// ---------------------------------------------------------------------------
inline uintptr_t gameBaseOffset() {
	static uintptr_t offset = (uintptr_t)GetModuleHandle(NULL) - 0x00400000;
	return offset;
}

inline uintptr_t aslr(uintptr_t addr) {
	return addr + gameBaseOffset();
}
...
#define SET_CLIENT_STATIC(x, y) decltype(x) x = *reinterpret_cast<std::add_pointer<std::remove_reference<decltype(x)>::type>::type>(aslr(y));
...
#define INITIALIZE_VTABLE_DATAFROM(NEWVTABLE, SRC) static int initialize_##NEWVTABLE = initVtableDataFrom(NEWVTABLE, (void*)aslr(SRC), sizeof(NEWVTABLE));
...
template<std::size_t Address, class newMethod_t, class original_t>
original_t HookStorage<Address, newMethod_t, original_t>::original = (original_t)aslr(Address);
...
namespace soe {
	...
	public:
		...
		static storage_t* allocate(std::size_t objectCount) {
			static strallocator_t strallocator1 = reinterpret_cast<strallocator_t>(aslr(SOEALLOCATOR_ADDRESS));
		...
		static void deallocate(storage_t* address, std::size_t objectCount) {
			static strdeallocator1_t deall1 = reinterpret_cast<strdeallocator1_t>(aslr(STRDEALLOCATOR1_ADDRESS));
			deall1(address);
		}
	};
	...
	public:
		...
		static storage_t* allocate(std::size_t objectCount) {
			if ((objectCount * sizeof(storage_t)) > 0x80) {
				...
				static strallocator2_t strallocator2 = reinterpret_cast<strallocator2_t>(aslr(STRALLOCATOR_ADDRESS));
		...
		static void deallocate(storage_t* address, std::size_t objectCount) {
			if ((objectCount * sizeof(storage_t)) > 0x80) {
				...
				static strdeallocator2_t deall = reinterpret_cast<strdeallocator2_t>(aslr(STRDEALLOCATOR2_ADDRESS));
