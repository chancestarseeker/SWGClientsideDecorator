...
Object::dyn_cast_t BaseHookedObject::soe_rt_dynamic_cast_func = nullptr;
...
PVOID BaseHookedObject::dynamicCast(PVOID inptr, const PVOID SrcType, const PVOID TargetType) {
	...
	/* lazy-init the function pointer with ASLR-adjusted address */
	if (!soe_rt_dynamic_cast_func) {
		soe_rt_dynamic_cast_func = (dyn_cast_t)aslr(SOE_RTDYNAMIC_CAST_ADDRESS);
	}
	/* RTTI pointers also need ASLR adjustment since they point into the game's .rdata section */
	return soe_rt_dynamic_cast_func(inptr, 0, (PVOID)aslr((uintptr_t)SrcType), (PVOID)aslr((uintptr_t)TargetType), 0);
}
